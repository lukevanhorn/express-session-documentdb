...
DocumentDBSessionStore.prototype.destroy = function(sid, callback) {
    ...
    self.readOrCreateDatabase(self.databaseId, function(database) { 
        self.readOrCreateCollection(database, self.collectionId, function(collection) {
            self.client.queryDocuments(collection._self, 'SELECT * FROM root r WHERE r.id="' + sid + '"').toArray(function (e, results) {
                if (e || !results || !results[0]) {
                    return callback(e);
                }

                self.client.deleteDocument(results[0]._self,function(e) {
                    callback(e);
                });
            });
        });
